...
# Configurations
# Live webcam with heatmap
# python main_with_heatmap.py --enable-heatmap

# Process video file with heatmap
# python main_with_heatmap.py --source videos/myvideo.mp4 --enable-heatmap

# Full demo: process video, record output, generate person clips AND heatmaps
# python main_with_heatmap.py --source input.mp4 --record --make-person-videos --enable-heatmap --person-fps 12
...
from face_heatmap_integration import FaceTrackingHeatmap
...
HEATMAP_OUTPUT_DIR = os.path.join(SCRIPT_DIR, "heatmaps")
...
def generate_all_heatmaps(heatmap_tracker, background_frame=None):
    """Generate all heatmap visualizations at end of processing."""
    ...
    # Global heatmap (NO BACKGROUND)
    print("\n1. Global heatmap (all faces)...")
    heatmap_tracker.visualize_global_heatmap(
        save_path=os.path.join(HEATMAP_OUTPUT_DIR, 'global_heatmap.png'),
        background_frame=background_frame,
        colormap='hot',
        alpha=0.7,
        show_background=False  # ← NO BACKGROUND
    )
    
    # All trajectories on one plot (NO BACKGROUND)
    print("2. All trajectories combined...")
    heatmap_tracker.visualize_all_trajectories(
        save_path=os.path.join(HEATMAP_OUTPUT_DIR, 'all_trajectories.png'),
        background_frame=background_frame,
        show_background=False  # ← NO BACKGROUND
    )
    
    # Global combined view (NO BACKGROUND)
    print("3. Global combined view (heatmap + trajectories)...")
    heatmap_tracker.create_combined_visualization(
        save_path=os.path.join(HEATMAP_OUTPUT_DIR, 'global_combined.png'),
        background_frame=background_frame,
        show_background=False  # ← NO BACKGROUND
    )
    
    # Per-person visualizations
    if heatmap_tracker.person_heatmaps:
        print(f"\n4. Generating per-person heatmaps ({len(heatmap_tracker.person_heatmaps)} persons)...")
        for person_id in sorted(heatmap_tracker.person_heatmaps.keys()):
            print(f"   → Person {person_id}...")
            
            # Individual heatmap (NO BACKGROUND)
            heatmap_tracker.visualize_person_heatmap(
                person_id,
                save_path=os.path.join(HEATMAP_OUTPUT_DIR, f'person_{person_id}_heatmap.png'),
                background_frame=background_frame,
                show_background=False  # ← NO BACKGROUND
            )
            
            # Individual trajectory (NO BACKGROUND)
            heatmap_tracker.visualize_person_trajectory(
                person_id,
                save_path=os.path.join(HEATMAP_OUTPUT_DIR, f'person_{person_id}_trajectory.png'),
                background_frame=background_frame,
                show_background=False  # ← NO BACKGROUND
            )
            
            # Combined view (NO BACKGROUND)
            heatmap_tracker.create_combined_visualization(
                person_id,
                save_path=os.path.join(HEATMAP_OUTPUT_DIR, f'person_{person_id}_combined.png'),
                background_frame=background_frame,
                show_background=False  # ← NO BACKGROUND
            )
    ...
    report = heatmap_tracker.generate_summary_report(
        save_path=os.path.join(HEATMAP_OUTPUT_DIR, 'tracking_report.txt')
    )
    print("\n" + report)
    
    # Export raw data
    print("\n6. Exporting raw heatmap data...")
    heatmap_tracker.export_heatmap_data(
        save_dir=os.path.join(HEATMAP_OUTPUT_DIR, 'raw_data')
    )
    
    print("\n" + "=" * 70)
    print(f"✓ All heatmap visualizations saved to: {HEATMAP_OUTPUT_DIR}")
    print("=" * 70)


def run(source=0, save_video=False, output_video_name=None, enable_heatmap=False):
    ...
    if isinstance(source, str) and os.path.isfile(source):
        ...
        # Try with CAP_DSHOW on Windows, fall back to default
        cap = cv2.VideoCapture(source)
        if not cap.isOpened():
            try:
    ...
    if not cap.isOpened():
        ...
        print(f"Tried to open: {source}")
        print("\nTroubleshooting:")
        print("  1. Check the file path is correct")
        print("  2. Make sure the file exists: ls -la", source if isinstance(source, str) else "")
        print("  3. Try using absolute path: --source /full/path/to/video.mp4")
    ...
    if enable_heatmap:
        print(f"\n✓ Heatmap tracking ENABLED (resolution: {width}x{height})")
        heatmap_tracker = FaceTrackingHeatmap(width=width, height=height, sigma=40, enable_per_person=True)
    ...
    while True:
        ...
        # Save a background frame for heatmap overlay (mid-video frame)
        ...
        # === Update heatmap ===
        if enable_heatmap and heatmap_tracker is not None:
            heatmap_tracker.update(bboxes, ct.visible)
        ...
        for pid, bbox in bboxes.items():
            ...
            if frame_idx % 5 == 0:
                ...
                if len(existing) < 10:
                    ...
                    append_metadata(pdir, {"time": now_ts(), "frame": frame_idx, "bbox": [int(x), int(y), int(w), int(h)], "quality_score": float(quality)})
                else:
                    ...
                    if quality > existing[worst_idx].get("quality_score", 0):
                        ...
                        existing[worst_idx] = {"time": now_ts(), "frame": frame_idx, "bbox": [int(x), int(y), int(w), int(h)], "quality_score": float(quality)}
    ...
    # === Generate Heatmaps ===
    if enable_heatmap and heatmap_tracker is not None:
        generate_all_heatmaps(heatmap_tracker, background_frame)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Face Recognition & Tracking System with Heatmap Generation")
    ...
    parser.add_argument("--enable-heatmap", action="store_true",
                        help="Enable heatmap generation and tracking visualization")
    ...
    run(source=source, save_video=args.record, output_video_name=args.output, enable_heatmap=args.enable_heatmap)
